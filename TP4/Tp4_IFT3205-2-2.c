...
void restore(float**,float**,float**,int,int,float,float);
...
int main(int argc,char **argv)
 {
  ...
  restore(mat_rest, degradedImg, degradedImgI, length, width, size, variance_noise);

  adjustGrey(mat_rest, length, width);
  printf("\n ISNR: %f", isnrResult(mat_img, mat, mat_rest, length, width));
...
void restore(float** matRestore, float** matGR, float** matGI, int length, int width, float l, float var){
  float denom;
  var /= (length*width);
  ...
  ModSquare(squareModFPrev, matGR, matGI, length, width);
