...
int main(int argc,char **argv)
 {
  ...
  /*Mul by distance to center*/
  for (i=0; i < length; i++) {
    for (j=0; j < width; j++) {
      int d_i = abs(i - length/2);
      int d_j = abs(j - length/2);
      int i2 = d_i * d_i;
      int j2 = d_j * d_j;
      MatriceImgR[i][j] = MatriceImgR[i][j] * sqrt(i2 + j2);
      MatriceImgI[i][j] = MatriceImgI[i][j] * sqrt(i2 + j2);
    }
  }
