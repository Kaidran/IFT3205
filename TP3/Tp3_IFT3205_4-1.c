...
int main(int argc,char **argv)
 {
  ...
  float** MatriceTopOriginal=fmatrix_allocate_2d(length+n, width);

  for(i=0; i<n; i++)
    for(j=0; j<width; j++){
      MatriceImg3[i][j] = averageValue(MatriceImg1, j, length, n);
      MatriceTopOriginal[i][j] = WHITE;
    }
  ...
  for (i=0;i<20;i++) {
    filter_HT(MatriceTopOriginal, MatriceImg3, length+n, width,120,width,0,28*i,lam);
    adjust(MatriceImg3,length+n,width);
    lam /= 2;
  }

  SaveImagePgm(NAME_IMG_OUT,MatriceImg3, length+n, width);
...
void filter_HT(float** img_init, float** img_curr, int len, int wid,
               int sizei, int sizej, int offset_i, int offset_j, float lam)
{
  int i,j;

  float** MatriceTmpR=fmatrix_allocate_2d(sizei,sizej);
  float** MatriceTmpI=fmatrix_allocate_2d(sizei,sizej);
  float** MatriceTmpM=fmatrix_allocate_2d(sizei,sizej);
  ...
  DFT(MatriceTmpR, MatriceTmpI, sizei, sizej);
  Mod(MatriceTmpM,MatriceTmpR,MatriceTmpI,sizei,sizej);
  ...
  IDFT(MatriceTmpR, MatriceTmpI, sizei, sizej);

  for (i=offset_i; i < sizei+offset_i; i++) {
    for (j=offset_j; j < sizej+offset_j; j++) {
