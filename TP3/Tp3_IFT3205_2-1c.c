...
int main(int argc,char **argv)
 {
  ...
  float** MatriceImg2R=fmatrix_allocate_2d(length*scaling,width*scaling);
  float** MatriceImg2I=fmatrix_allocate_2d(length*scaling,width*scaling);
  ...
  for(i=0;i<length*scaling;i++)
    for(j=0;j<width*scaling;j++)
      {
  ...
  //Interpolation 
  img2d_IS(MatriceImg1R, MatriceImg1I, MatriceImg2R, MatriceImg2I, length, width);

  /*IFFTD*/

  IFFTDD(MatriceImg2R, MatriceImg2I, length*scaling, width*scaling);
  CenterImg(MatriceImg2R, length*scaling, width*scaling);

  /*Pour visu*/
  Recal(MatriceImg2R, length*scaling, width*scaling);

  //Sauvegarde
  SaveImagePgm(NAME_IMG_OUT,MatriceImg2R, length*scaling, width*scaling);
